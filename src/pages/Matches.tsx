...
const Matches = () => {
  ...
  const handleStatusUpdate = async (id: string, newStatus: string) => {
    try {
      await updateMatchStatus(id, { status: newStatus as any });
    } catch (error) {
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Found listings matching your criteria ({matches.length} total)</CardDescription>
        </CardHeader>
        <CardContent>
          {matches.length > 0 ? (
                ...
                <div key={match.id} className="border rounded-lg p-4 space-y-3">
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-4 mt-2 text-sm text-gray-600">
                        <span className="font-semibold">${match.price?.toLocaleString()}</span>
                  ...
                  <div className="flex gap-2 pt-2">
                    ...
                    {match.status === 'new' && (
                      <Button 
                        size="sm" 
                        onClick={() => handleStatusUpdate(match.id, 'purchased')}
                      >
                        Mark Purchased
                      </Button>
                    )}
